...
CreateWorkerTask::CreateWorkerTask(std::shared_ptr<TaskEmitter> taskEmitter) : Task("Mineral Gathering Task", taskEmitter) {
    ...
    BT_NODE* pActionGatherNearestMineral = new BT_ACTION_EXECUTE_METHOD("Action execute gatherNearestMaterial", m_taskBT.get(), std::bind(trainWorker, std::placeholders::_1, this));
}
